---
name: code-solver
description: 快速解答算法题并审查代码，先生成中文题解，再生成 Java、Python、C、C++ 或 TypeScript 的题目代码；detail 额外生成可运行测试代码，review 支持用户上传或指定代码规范，生成代码审查报告。用于各算法平台刷题、公司题库、刷题复盘。
---
...
- 优先生成完整题解并写入最终归档，再生成代码。
- 不创建临时文件，不移动文件，不回读刚写入的文件。
- 不编译、不运行、不在终端验证或自动修复生成代码；由用户测试。
- 只在 review 模式按照“Review 规范”读取一个规范源。
- 无需读取 spec 文件。
...
读取当前模式对应的模板文件（见"题解模板"），按占位符填充后一次生成完整 Markdown，直接写入 `note`。不要先生成 JSON、草稿或分章节补写。
...
| 模式 | 模板文件 | 读取时机 |
|---|---|---|
| fast | `<skill_dir>/references/fast-template.md` | solve fast 第一轮 |
| detail | `<skill_dir>/references/detail-template.md` | solve detail 第一轮 |
...
| 模式 | 模型生成轮次 | 目录调用 | 模板读取 | 文件写入 | 产物回读 | 运行验证 |
|---|---:|---:|---:|---:|---:|---:|
| fast | 2 | 1 | 1 | 2 | 0 | 0 |
| detail | 2 | 1 | 1 | 3 | 0 | 0 |
| review | 1 | 1 | 0 | 1 | 0 | 0 |
