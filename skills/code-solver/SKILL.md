...
从当前输入直接确定平台、题号、题名、语言和笼统题型：

- **平台**：题源名称，如 `leetcode`、`nowcoder`、`acwing`、`huawei` 或内部题库名称；无法判断时用 `local`。
- **题号**：有标准题号时直接使用（如 `206`、`NC1`、`HJ1`）；无标准题号时从题名或原题信息中提取关键词作为短标识（不超过 20 字符），不得将整段题面作为题号。
- **题名**：从题面提取简洁中文名；无法提取时用 `未命名题目`。
...
## 核心思路
{题目本质、约束、关键状态或不变量、选择数据结构、核心思路；保持简洁}
...
官方样例验证
- 官方样例：{基于官方样例补充算法推导步骤，没有构建简单样例，方便用户理解}
- 必要边界：{仅补充能暴露错误的边界，可省}
